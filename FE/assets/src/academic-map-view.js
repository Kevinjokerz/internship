async function loadHeadContent() {
    ...
    try {
        const academicMapData = await fetchAcademicMapInfo(academicMapId);
        console.log(academicMapData);

        const content = document.getElementById("academic-map-text");
        const header = document.createElement("h1");
        header.textContent = academicMapData.college.collegeName;
        const header2 = document.createElement("h2");
        const para = document.createElement("p");
        para.innerText = "Four-Year Academic Map 2024-2025"
        header2.textContent = `${academicMapData.degreeType}, ${academicMapData.major}`
        content.appendChild(header)
        content.appendChild(header2)
        content.appendChild(para)
    } catch (error) {
        console.error("Error loading academic map data:", error);
    }
}
...
async function loadDegreePlan() {
    ...
    try {
        const planner = await fetchCourseForSampleAM(academicMapId)
...
function showModal(course) {
  ...
  if (course.prerequisites && course.prerequisites.length > 0) {
    const prereqCodes = course.prerequisites.map((p) => {
      ...
      if (!pc) return "";
      const pcTitle = pc.courseNo
        ? `${pc.courseCode} ${pc.courseNo}`
        : pc.courseCode;
...
for (const year of years) {
  ...
  fallCourses.sort((a, b) => a.sortOrder - b.sortOrder);
  springCourses.sort((a, b) => a.sortOrder - b.sortOrder);
  ...
  fallHeader.textContent =
    fallPlanner && fallPlanner.semester && fallPlanner.semester.semesterName
      ? `${fallPlanner.semester.semesterName} ${fallPlanner.season}`
      : `Semester 1 Fall`;
  fallHeader.colSpan = 3;
  headerRow.appendChild(fallHeader);

  const springHeader = document.createElement("th");
  springHeader.textContent =
    springPlanner && springPlanner.semester && springPlanner.semesterName
      ? `${springPlanner.semester.semesterName} ${springPlanner.season}`
      : `Semester 2 Spring`;
